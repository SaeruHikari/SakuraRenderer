...
namespace SGraphics
{
	class SCubeMapConvPass : public SDx12Pass
	{
	public:
		SCubeMapConvPass(ID3D12Device* device)
			:SDx12Pass(device)
		{
		...
		{
			if (PS == nullptr)
				PS = d3dUtil::CompileShader(L"Shaders\\PBR\\Pipeline\\SkyPass.hlsl", nullptr, "PS_Conv", "ps_5_1");
			if (VS == nullptr)
